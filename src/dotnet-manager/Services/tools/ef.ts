    ...
    try {
      if (this.useGlobalDotnetEf) {
        ...
        if (!fs.existsSync(dotnetEfPath)) {
          throw new Error(
            `The dotnet-ef executable was not found in the global tools directory: ${dotnetEfPath}`
          )
        }
        ...
        try {
          ...
          throw new Error(
            'Unable to locate executable file: dotnet-ef. Please verify the PATH environment variable.'
          )
        }
      } else {
