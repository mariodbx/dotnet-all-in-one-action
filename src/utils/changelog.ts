...
/**
 * Interface for generating changelogs.
 *
 * This interface defines the contract for any class that implements changelog generation functionality.
 */
...
/**
 * Implementation of the IChangelogGenerator interface for generating changelogs.
 *
 * This class provides functionality to generate a changelog by categorizing commit messages
 * based on keywords.
 */
class ChangelogGenerator implements IChangelogGenerator {
  private readonly changelogFile: string

  constructor(changelogFile: string = 'changelog.txt') {
   ...
   *
   * @example
   * const regex = this.buildKeywordRegex('fix,bug,patch');
   ...
   * const regex = /fix/;
   * console.log(this.categorize(commits, regex));
   * // Output:
   * // fix: issue 1
   * // fix: issue 2
   *
   ...
   * generator.generate().then((changelog) => console.log(changelog));
   *
   * @remarks
   ...
   * provided via inputs or defaults. The changelog is written to a file specified in the constructor.
   *
   * Inputs:
  ...
  public async generate(): Promise<string> {
    ...
    const changelog = [
      [
        '### Major Changes',
        this.buildKeywordRegex(getInputOrDefault('major_keywords', 'major'))
      ],
      [
        '### Minor Changes',
        this.buildKeywordRegex(getInputOrDefault('minor_keywords', 'minor'))
      ],
      [
        '### Patch/Bug Fixes',
        this.buildKeywordRegex(
          getInputOrDefault('patch_keywords', 'patch,bug-fix,bug fix')
        )
      ],
      [
        '### Hotfixes',
        this.buildKeywordRegex(getInputOrDefault('hotfix_keywords', 'hotfix'))
      ],
      [
        '### Additions',
        this.buildKeywordRegex(getInputOrDefault('added_keywords', 'added'))
      ],
      [
        '### Dev Changes',
        this.buildKeywordRegex(getInputOrDefault('dev_keywords', 'dev'))
      ]
    ]
      .map(
        ([label, regex]) =>
    ...
    await fs.writeFile(this.changelogFile, changelog, 'utf8')
...
export async function generateChangelog(): Promise<string> {
  return await changelogGenerator.generate()
}
