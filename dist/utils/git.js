...
export async function getLatestCommitSubject() {
    ...
    if (result.exitCode !== 0) {
        throw new Error(`Failed to retrieve the latest commit subject: ${result.stderr}`);
    }
 ...
 */
export async function gitRestore(paths) {
 ...
 */
export async function gitCreateAndSwitchBranch(branchName) {
    const result = await exec.getExecOutput('git', ['checkout', '-b', branchName]);
    if (result.exitCode !== 0) {
        throw new Error(`Failed to create and switch to branch '${branchName}': ${result.stderr}`);
    }
}
/**
 ...
 */
export async function gitDeleteBranch(branchName) {
 ...
 */
export async function gitMerge(branchName) {
 ...
 */
export async function gitReset(commitHash, hard = false) {
    const args = ['reset', hard ? '--hard' : '--soft', commitHash];
    const result = await exec.getExecOutput('git', args);
    if (result.exitCode !== 0) {
        throw new Error(`Failed to reset to commit '${commitHash}': ${result.stderr}`);
    }
}
