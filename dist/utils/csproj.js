 ...
 * @example
 * const csprojPath: string = await findCsprojFile(2, 'MyProject.csproj', false);
 ...
 */
export async function findCsprojFile(csprojDepth, csprojName, showFullOutput) {
    const findCmd = `find . -maxdepth ${csprojDepth} -name "${csprojName}" | head -n 1`;
    return await runCommand('bash', ['-c', findCmd], {}, showFullOutput);
}
/**
 ...
 */
export function extractVersionSuffix(csprojContent) {
 ...
 */
export function updateVersionSuffixContent(csprojContent, newSuffix) {
    const suffixRegex = /<VersionSuffix>([^<]+)<\/VersionSuffix>/;
 ...
 */
export function removeVersionSuffix(csprojContent) {
