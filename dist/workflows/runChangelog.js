 ...
 */
async function getExistingRelease(token, owner, repo, tag) {
...
export async function runChangelog() {
    try {
        ...
        let releaseResponse = await getExistingRelease(token, owner, repoName, version);
        if (!releaseResponse && inputs.runRelease) {
            ...
            core.info('No existing release found and release creation not enabled; skipping asset upload.');
            return;
        }
        else {
        ...
        if (assets.length > 0) {
            ...
            core.info(`Uploading ${assets.length} assets to release ID ${releaseId}...`);
        ...
        core.info('Changelog and release asset upload process completed successfully.');
    }
    catch (error) {
