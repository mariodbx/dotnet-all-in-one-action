    ...
    try {
        ...
        if (inputs.dockerComposeFiles) {
            ...
            for (const file of dcFiles) {
                ...
                await dockerManager.pushCompose(newVersion, composeImages, inputs.pushWithVersion, inputs.pushWithLatest);
            }
        }
        ...
        if (inputs.dockerfiles) {
            ...
            for (let i = 0; i < dockerfiles.length; i++) {
                ...
                const imageWithVersion = await dockerManager.buildDocker(dockerfiles[i], contexts[i], newVersion, dockerfileImages[i]);
