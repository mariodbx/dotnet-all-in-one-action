...
export class ArtifactManager {
     ...
     */
    constructor(artifactName, retentionDays) {
     ...
     */
    async upload(resultFilePath, resultFolder) {
        if (fs.existsSync(resultFilePath)) {
            core.info(`Uploading test result file from ${resultFilePath}...`);
            const artifactClient = new artifact.DefaultArtifactClient();
            try {
                const { id, size } = await artifactClient.uploadArtifact(this.artifactName, [resultFilePath], resultFolder, { retentionDays: this.retentionDays });
...
export async function uploadTestArtifact(resultFilePath, resultFolder) {
    const artifactUploader = new ArtifactManager('test-results', 7);
