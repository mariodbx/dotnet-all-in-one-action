...
import { getLatestCommitSubject, extractVersionFromCommit, findCsprojFile, extractVersionFromCsproj } from './changelog/git-utils.js';
import { getActionInput } from './utils/input-utils.js';
...
export async function run() {
    try {
        ...
        const useCommitMessage = getActionInput('use_commit_message', 'true').toLowerCase() === 'true';
        let version = null;
        if (useCommitMessage) {
            ...
            core.info(`Searching for csproj file with pattern "${csprojName}" at max depth ${csprojDepth}`);
